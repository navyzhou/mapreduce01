 ...
 * @author navy
 */                                                           // 自定义类型
public class SaleMap extends Mapper<LongWritable, Text, Text, SaleWritable>{
	@Override
	protected void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
		...
		if (value != null && value.getLength()>0) { // 如果数据不为空
			...
			if (strs.length == 3) { // 如果长度为5，说明是合理的数据   1001	张三	200	技术部	2018-01  
				// 将参数封装到自定义数据类型中
				SaleWritable sw = new SaleWritable();
				sw.setPrice(Double.parseDouble(strs[2]));
				context.write(new Text(strs[0]), sw);
			}
		}
	}
}	
